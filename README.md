...
>
> **Status:** v0.9.0 ship. The installer is a `pnpm dlx opencode-ship@latest <cmd>` workflow. Five idempotent CLI commands manage a managed-file lock, a transactional promoter, and a compiled ESM plugin that registers the canonical nine `delivery_*` tools. Every command accepts `--profile <name>` (`core` default, `engineering` opt-in`). The `core` profile ships only opencode-ship-authored content. The `engineering` profile installs the Plan Mode permission block under `agent.plan.permission`; the two engineering skill assets (`triage`, `grill-with-docs`) are still placeholders and the full Matt/Superpowers workflows are not vendored yet. The M3 task loop and parallel Standards/Spec Ready gate helpers are implemented as source-only modules with passing tests, but they are not yet wired into the shipped plugin or CLI dispatch. v0.9 ships the transition matrix smoke (core omits engineering, engineering adds engineering, lock tracks the active profile) required by issue #24.
>
> **Work-in-progress notice (2026-08-05).** The release/1.0-completion branch contains partial source-only helpers for the planned `1.0.0` runtime, but production wiring is not complete. A read-only audit found only Task 1 of the canonical plan passing; Tasks 2–15 are not production-ready. Local `v0.10.0` and `v1.0.0` tags exist as placeholders only; they are not published and must not be pushed until the formal registry dogfood passes. No `0.10.0` or `1.0.0` is published yet; consumers should keep using `opencode-ship@0.9.0`.
