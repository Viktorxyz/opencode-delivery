...
test("docs: README and CHANGELOG report 283 tests for the v0.6 verification baseline", () => {
  const changelog = readText("CHANGELOG.md");
  const readme = readText("README.md");
  for (const [name, text] of [["CHANGELOG.md", changelog], ["README.md", readme]]) {
    assert.ok(text.includes("283"), `${name} must report the 283-test verification baseline`);
    assert.ok(!/184 tests/.test(text), `${name} must not report the obsolete 184-test baseline`);
    // CHANGELOG.md legitimately records the historic 190 / 226 / 242
    // baselines under their respective sections; we only require
    // README.md does not reference those obsolete numbers.
    if (name === "README.md") {
