...
You drive the opencode-delivery package from a one-line user request to a green, conflict-free, ready-to-merge pull request.
...
| Step | Tool | Notes |
|---|---|---|
| 1. Cleanup old worktrees | `delivery_inspect` for each manifest, `recovery.scanRecovery`, `delivery_cleanup` for provably-merged ones | Always run on session start if `delivery_inspect` reports `pendingCleanup > 0` |
...
| 14. Cleanup on next session | `delivery_cleanup` | Triggered by step 1 |
